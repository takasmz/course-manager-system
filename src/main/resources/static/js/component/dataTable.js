    ...
    return {
        ...
        load: function (options) {
            ...
            var defaultOptions = {
                ...
                selectExport: false,
                //各个组件的位置配置
                dom: '<"wapper"rt><"bottom"<"pull-left page-length"l><"pull-left"i><"pull-left"B><"pull-right"p<"pull-right spec-num">>>',
                buttons: [
//                    {
//                        extend: 'print',
//                        text: '打印'
...
//                    }
            ...
            if (opt.showIndex) {
                ...
                table.jump.node=$(table.nodes()).parents('.dataTables_wrapper').find('.spec-num');

                table.jump.node.html('跳到<input type="text" class="txt js-cust-txt">页<input type="button" class="sbtn js-cust-btn" value="确定">');

                //指定页码跳转
                table.jump.jumpPage=function () {
                    var jumpTxtNode=table.jump.node.find('.js-cust-txt');
                    var num=jumpTxtNode.val()-1;
                    if(!isNaN(num)&&num>=0){
                ...
                $(document).off('click.jumpPage'+options.el).on('click.jumpPage'+options.el,options.el+'_wrapper .js-cust-btn',function () {
                    table.jump.jumpPage();
                });

                $(document).off('keydown.jumpPage'+options.el).on('keydown.jumpPage'+options.el,options.el+'_wrapper .js-cust-txt',function (event) {
                    if(event.keyCode ==13){
                        table.jump.jumpPage();
                    }
                });
